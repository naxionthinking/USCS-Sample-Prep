...
check_negative_sp_codes <- function(df, sp_code_no_negatives=SP_CODES_NO_NEGATIVES){
  ...
  if (nrow(unauthorized_negatives) > 0) {
    validation_results$unauthorized <- sprintf(
      "SP Code %s: %d/%d records (%.1f%%) have unauthorized zero/negative spend",
  ...
  if (nrow(missing_negatives) > 0) {
    validation_results$missing <- sprintf(
      "SP Code %s: Expected zero/negative spend not found in %d records",
