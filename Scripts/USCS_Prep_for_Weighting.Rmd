...
all_base <- load_base() %>%
  # Blank out unnecessary personal information
  mutate(nm_pfx_tx  = "",
         nm_suff_tx  = "",
         best_addr_line1_tx = "",
         best_addr_line2_tx = "",
         sal_tx = "",
         home_phone_ln_no = "",
         new_smart_rev = as.numeric(new_smart_rev_char)) %>%
...
all_spend <- load_spend() %>%
  ...
  filter(substring(customer_id, 1, 1) != " ") # Remove blank space at start of ID
```
...
12) Run ReadStep1.sas from line 295 (BestDMA Frequency) to line 300 (output
    to 'DMA Check') Frequency data gets written into 'DMA Check' columns F
    & G. Verify that everything got written okay. If all DMAs are
    acceptable, 'DMA Check'!E215 should say "All Good" and 'Main List'!F37
    should say "Okay" If they say something different, resolve the problem
    or note the exception

    **I'M NOT WHATS HAPPENING HERE TO BE ABLE TO CODE IT UP**
