...
# Check Counts
main_marketer_codes_check <- main_sp_codes %>% 
...
augment_cc_check <- USCS_Sample %>% filter(!is_main) %>% 
  group_by(cell_code) %>% summarize(count_received = n()) %>% 
...
invalid_dmas <- USCS_Sample %>% 
 filter(!best_dma_cd %in% valid_dmas) %>% 
 group_by(best_dma_cd) %>% summarize(count = n())
...
log_check_result(
  ...
  check_type = "validity",
  data = invalid_dmas,
  row_message = "DMA Code {row$best_dma_cd}: {row$count} occurrences"
)
```
...
to_remove_tenure_spend_helper <- USCS_Sample %>% filter(!is_main) %>% 
  select(gmpi_base_cust_id, cell_code, tenure_var, 
...
# We save a copy of the dataset here before the removals, so it's easy to not remove something if asked.
save(USCS_Sample, file="../Data/USCS_Sample_Preremovals-{Sys.Date()}.Rdata")
...
USCS_Sample <- USCS_Sample %>% 
  ...
  remove_bad_sample(to_remove_miss_spend, "Missing Tenure") %>% 
...
## Save Intermediate Data and Clear Workspace
```{r Save Workbook}
...
```{r Save Post-Removals}
# Store sample dataset after removals
# save(USCS_Sample, file=f_str("../Data/USCS_unweighted_sample_{MONTH}_{YEAR}-{Sys.Date()}.Rdata"))
```
...
# save(USCS_Final, file=f_str("../Data/USCS_final_{MONTH}_{YEAR}.Rdata"))
```
...
# load(f_str("../Data/USCS_final_{MONTH}_{YEAR}.Rdata"))
```
