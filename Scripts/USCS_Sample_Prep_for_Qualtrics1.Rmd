...
```{r}
wrong_yob <- USCS_Sample %>% filter(!is.na(raw_age) & (raw_age < 18 | raw_age > 120)) %>% 
  select(gmpi_base_cust_id, marketer_code, naw_cell_code, raw_age, setup_dt, card_anniv_dt)

if (nrow(wrong_yob) > 0){
  warning(f_str('There were {nrow(wrong_yob)} people who are too young (<18) or too old(>120'))
  ...
  writeData(Diagnostic_WB, "Removed_YOB", wrong_yob) 
}
```
...
if (nrow(tenure_spend_check) > 0){
  warning(f_str("There are {nrow(tenure_spend_check)} people flagged for deletion in the augment file due to bad account spends or tenures. These will be found in the `Files_to_send/USCS_Diagnostics_MONTH_YEAR.xlsx` file. Verify with the PM if they are okay with these being removed in the email at the end of this script"))
...
writeData(Diagnostic_WB, "Removed_Tenure_or_Spend", tenure_spend_check) 
saveWorkbook(Diagnostic_WB, file = f_str("../Files_to_send/USCS_Diagnostics_{MONTH}_{YEAR}.xlsx"), overwrite = TRUE)
```
...
```{r}
# Remove bad YOB
USCS_Main <- USCS_Main %>% anti_join(wrong_yob, by = 'gmpi_base_cust_id') # Line to comment out if PM does not want these samples removed
USCS_Augment <- USCS_Augment %>% anti_join(wrong_yob, by = 'gmpi_base_cust_id') # Line to comment out if PM does not want these samples removed
