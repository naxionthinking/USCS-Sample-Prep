...
main_code_freq %>%  
  write_csv(f_str("../Files_to_send/Main_Code_Freqs_{MONTH_SHORT}_{YEAR}.csv")) 
```
...
cell_code_freq %>% 
  write_csv(f_str("../Files_to_send/Cell_Code_Freqs_{MONTH_SHORT}_{YEAR}.csv"))
...
all_base %>% write_csv("../Data/all_base_{MONTH_SHORT}_{YEAR}.csv" %>% f_str())
all_spend %>% write_csv("../Data/all_spend_{MONTH_SHORT}_{YEAR}.csv" %>% f_str())
...
if ("A" %in% tab$spend) {
  ...
  for (code in bad_sp_code_a) {
    warning("Spending category A found in Sp Code: {code}" %>% f_str())
    cat("\n")}
} else{
...
if (sum(min_acct_spend_a$spend_zero > 0)){
  bad_min_spend <- min_acct_spend_a$marketer_code[min_acct_spend_a$spend_zero]
    for (code in bad_min_spend) {
      warning("Minimum Account spend is not greater than zero for requested Sp Code: {code}" %>% f_str())
      cat("\n")}
} else{
...
if (length(check_a2) == nrow(check_a)){ # Everything should be in check2
  message("SUCCESS: All Sp codes are in only two of the lists")
    ...
    for (code in check_a1) {
      warning("Sp Code: {code} is only in one of the lists. Look at description above for what to do" %>% f_str())
      cat("\n")}
  warning("")
} else if (length(check_a3) > 0 ){
    for (code in check_a3) {
      warning("Sp Code: {code} is in all three lists" %>% f_str())
      cat("\n")}
...
```{r }
write_csv(all, "../Data/all_{MONTH_SHORT}_{YEAR}.csv" %>% f_str())
write_csv(main, "../Data/main_{MONTH_SHORT}_{YEAR}.csv" %>% f_str())
```
...
if (n_flags > 0){
  ...
  cat("\n")
  warning("Find specifics in file `Tenure_or_Spend_Flagged_{MONTH_SHORT}_{YEAR}.csv` in the Files_to_send folder" %>% f_str())
...
save(augment, main, file=f_str("../Data/unweighted_samples_{MONTH_SHORT}_{YEAR}.Rdata"))
```

```{r message=FALSE}
# IF YOU ARE RUNNING `USCS_Weighting.Rmd` right after this, run this chunk and then you DO NOT have to reload in the data with the step `load(f_str("../Data/unweighted_samples_{MONTH_SHORT}_{YEAR}.Rdata"))`
