...
cn68_market_codes <- USCS_Base[USCS_Base$cell_code == 'CN68',] %>% 
...
```{r}
wrong_yob <- USCS_Sample %>% filter(raw_age < 18 | raw_age > 120) %>% 
  select(gmpi_base_cust_id, marketer_code, naw_cell_code, raw_age, setup_dt, card_anniv_dt)

if (nrow(wrong_yob) > 0){
  warning(f_str('There were {nrow(wrong_yob)} people who are too young (<18)'))
  warning("These people are automatically removed from the sample and put into the Diagnostic Excel file. Verify with the PM if they are okay with these being removed. If not, comment out the anti_join line in this chunk when rerunning the script.")
  
  # Save the removals to the output workbook
  addWorksheet(output_wb, "Removed_YOB")
  writeData(output_wb, "Removed_YOB", wrong_yob) 
  # Remove the samples
  USCS_Sample <- USCS_Sample %>% anti_join(wrong_yob, by = 'gmpi_base_cust_id') # Line to comment out if PM does not want these samples removed
}
```
...
```{r}
tenure_spend_check <- USCS_Augment %>% 
...
if (nrow(tenure_spend_check) > 0){
  ...
  USCS_Augment <- USCS_Augment %>% anti_join(tenure_spend_check, by = "gmpi_base_cust_id") # Comment this out if not removing
} else{
...
**IF THEY SAY YOU SHOULD NOT REMOVE CERTAIN PEOPLE**
  - Go back to the section titled "Saving and Loading dataframe before removals"
  - Uncomment the line that loads in the `all_premoved.Rdata` file
    - This will overwrite the all dataframe in your environment to the version before anything was removed
    - Modify the code to not remove the people who should not be removed
      - Hopefully it's as simple as comment out one of the "ant-Join" statements
