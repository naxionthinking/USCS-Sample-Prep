...
options(scipen = 9999,# Remove scientific notation
        dplyr.summarise.inform = FALSE) #!! Suppress summarise info

knitr::opts_chunk$set(results = "asis", #!! write the text output as is, without nay markups
                      echo=F)  #!! knitr::opts_chunk$ses global options that apply to every chunk in your file
```


```{r}
# Load in the generated data sets from the first step (main and augment)
load(f_str("../Data/unweighted_samples_{MONTH_SHORT}_{YEAR}.Rdata"))
```


```{r}
main <- main %>% mutate(setx = 1)  #! %>% takes the output of the fn mutate and passes it into main
augment <- augment %>% mutate(setx = 2)  #! %>% takes the output of the fn mutate and passes it into augment

et_tab <- augment %>% 
  count(marketer_code, sp_code) %>% 
  spread(marketer_code, n, fill = 0) %>%  #!fill = fills missing with 0
  make_nice_table("Augmented Sample Sp Code vs Marketer Code Freqs")
  
```

```{r message=F}
combined <- full_join(main, augment) %>%  #! joins 2 tables together
...
weight_conditions <- read_excel(SAMPLE_PREP_PATH,
                      ...
                      na = c("", "n/a")) %>% 
  #! Drop rows containing missing values in column sp_code
  drop_na(sp_code)
...
gens <- weighted_sample %>% distinct(year_of_birth, generation) %>% 
...
```{r}
# Fill in the blank names with "Valued Card Member" because First and Last name are now included in the AllSample file. Also, is first name has more than 10 chars,  replace with "Valued Card" and last name with "Member"
weighted_sample <- weighted_sample %>% 
...
```{r}
final %>% write_csv("../Files_to_send/Amex_760_{MONTH}_{YEAR}_All_Sample_Frame_Information_Qualtrics_{Sys.Date()}.csv" %>% f_str(), na ="")

rm.all.but(keep=c('final')) #1 remove all files in directoty but 'final'
```
