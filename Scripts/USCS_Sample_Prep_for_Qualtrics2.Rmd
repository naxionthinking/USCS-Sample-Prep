...
USCS_New_Vars <- USCS_Weighted %>%
  mutate(
    cell_num = if_else(naw_type == 'AUGMENT', 
...
USCS_Final <- USCS_Final %>% mutate(
         ...
         across(c('BF_CARD_ANNIV_DT', "BF_CARD_ANNIV_YEAR", "BF_CARD_ANNIV_MONTH"), ~ifelse(. == "" | is.na(.), "REF", .)) # Make missing values "REF"
)
```
