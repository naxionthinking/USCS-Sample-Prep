...
**THE FOLLOWING CHUNK DOES NOT NEED TO BE RUN IF YOU JUST RAN STEP 1 AND STILL HAVE THE 'main' AND 'augment' DATAFRAMES IN YOUR ENVIRONMENT**
```{r}
...
message(f_str("There are {sum(is.na(combined$raw_fico))} missing values for raw_fico"))
```
...
final <- final %>% mutate(
         ...
         across(c('BF_CARD_ANNIV_DT', "BF_CARD_ANNIV_YEAR", "BF_CARD_ANNIV_MONTH"), ~ifelse(. == "" | is.na(.), "REF", .))
)
```

```{r}
MONTH_STR <- sprintf("%02d", MONTH_NO)
final %>% write_csv("../Files_to_send/AmexGABMUSCSSurvey_{YEAR}{MONTH_STR}_{Sys.Date()}.csv" %>% f_str(), na ="")
