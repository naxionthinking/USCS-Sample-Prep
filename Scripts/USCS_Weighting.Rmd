...
-   Writes that Augment Sample File

-   Writes the 'All Sample' file,

-   Starting in January 2013, we only had to send augment sample to
    Research Now,

-   In April 2013, we had to start sending something to Cheetah Mail
    again because CELL15 gets a 3:1 allocation,

-   In 2015 Cheetah Mail file started to get sent to Salesforce


```{r}
# Load in the generated data sets from the first step (main and augment)
load(f_string("../Data/unweighted_samples_{MONTH_SHORT}_{YEAR}.Rdata"))
```


```{r}
main <- main %>% mutate(setx = 1)
augment <- augment %>% mutate(setx = 2)

et_tab <- augment %>% 
  count(marketer_code, sp_code) %>%
  spread(marketer_code, n, fill = 0) %>% 
  make_nice_table("Augmented Sample Sp Code vs Marketer Code Freqs")
  
```

```{r message=F}
combined <- full_join(main, augment) %>% 
  mutate(cell_num = if_else(setx %in% c(2,3), 
                            str_extract(naw_cell_code, "[0-9]{2}"), NA_character_),
         creative = "")
```

```{r}
cell_no_freq <- combined %>% freq_table("cell_num", "Cell Number Frequency")

######## FLAG TO FREQ
message("There are {sum(is.na(combined$raw_fico))} missing values for raw_fico" %>% 
          f_string())
```
...
weights <- read_excel(SAMPLE_PREP_PATH,
...
weights <- weights %>% 
  # We first need to extract the sp codes
  # either the name of the segment has the designated codes
  # Or they are separately provided in another column SP_Code2
  
  # if there is an sp code in the helper column, we use it
  mutate(
    sp_code = if_else(str_detect(sp_code, "[0-9]{3}"),
                      str_extract_all(sp_code, " [0-9]{3}"),
  ...
  mutate(
        ...
        # So we need to account for all cases
         tenure_var = if_else(str_detect(tenure, "days"), 
                              # If tjos os tje days case. then divide by 30 and round up
                              (str_extract(tenure, "[0-9]+") %>% 
                                 as.numeric() / 30) %>% ceiling() %>% as.character(),
         ...
         # This looks like 
            ...
            # NUM < tenure_var ETC
         tenure_cond = case_when(
           is.na(tenure) ~ NA_character_,
         ...
         ordering = case_when(
                      str_detect(weighting_segment, "^TX")  ~ 1,
...
weighted_sample <- combined %>% 
  mutate(weighting_segment = case_when(!!!rlang::parse_exprs(weights$full_cond)))
```

- Verify splits were done correctly

```{r}
# Mean of account_spend by weighting_segment
weight_tab <- weighted_sample %>% 
...
weights_ncc_sp_code <- weighted_sample %>%
...
weighted_sample <- weighted_sample %>%
  mutate(
    ...
    bf_purch_apr_rt = case_when(
      purch_apr_rt > 0 ~ purch_apr_rt %>% round(2) %>% as.character(),
      purch_apr_rt == -1 ~ "",
      T ~ "REF"),
    
    # /**********CHECK TENURE*************/
    cv_cs_et_type = case_when(
      #Ops asked that SP code 158, new in Jan 2023, be called early tenure even though tenure_var values exceed 4 see email from Hunter 1/3/23*/
      marketer_code == "SP158" ~ 1, 
    ...
    #/*USE SMART_REV for Weighting Segment because CV_MARKET_SIZE wasn't working;
	  ...
	  #   note that cyear and cmonth are initizlized in step1*/
         ...
         generation2 = case_when(year_of_birth == "" ~ "",
...
```{r}
fico <- table(weighted_sample$fico_range, weighted_sample$bf_fico_range, useNA = "ifany") %>% make_nice_table("FICO Range vs BF FICO Range")
enroll <- table(weighted_sample$st_exp_enroll_in, weighted_sample$bf_st_exp_enroll_in, useNA = "ifany") %>% make_nice_table("st_exp_enroll_in vs bf_st_exp_enroll_in")
myca_flag <- table(weighted_sample$myca_flag, weighted_sample$bf_myca_flg, useNA = "ifany") %>% make_nice_table("myca_flag vs bf_myca_flg")
mr_in <- table(weighted_sample$mr_in, weighted_sample$bf_mr_in, useNA = "ifany") %>% make_nice_table("mr_in vs BF_mr_in")


cv_et <- weighted_sample %>% group_by_summary_table("cv_cs_et_type", "tenure_var") # *1 = tenure < 5, 2=tenure 5-15, 3=tenure > 15;

no_cv_cs_et <- weighted_sample %>% filter(is.na(cv_cs_et_type)) 

if (nrow(no_cv_cs_et) > 0) warning("Missing CV CS ET Type")


port_mc <- table(weighted_sample$marketer_code, weighted_sample$cv_portfolio, useNA = "ifany") %>% make_nice_table("cv_portfolio vs marketer_code")
fee_mc <- table(weighted_sample$marketer_code, weighted_sample$cv_amex_has_fee, useNA = "ifany") %>% make_nice_table("cv_amex_has_fee vs marketer_code")

if (sum(is.na(weighted_sample$account_number)) > 0) warning("Missing account spend found")

tier_cat <- table(weighted_sample$mr_tier_prog_cd, weighted_sample$cv_mr_category, useNA = "ifany") %>% 
  make_nice_table("mr_tier_prog_cd vs cv_mr_category")
aug_naw <- table(weighted_sample$cv_augment_sample, weighted_sample$naw_cell_code, useNA = "ifany") %>% 
...
```{r}
# Missing Full Name
miss_name <- weighted_sample %>% 
  filter(is.na(first_nm) & is.na(last_nm)) %>% 
  dplyr::select(customer_id,first_nm, last_nm)

if (nrow(miss_name) > 0){
  warning('Fully missing names found')
  print(miss_name)
} else{
  message("SUCCESS: No Fulling missing names")
}
```
- Fills in the blank names with "Valued Card Member" because First and Last name are now included in the AllSample file
```{r}
weighted_sample <- weighted_sample %>% 
...
```{r message=F}
# *Read in Subject Line Code Info from file which Brent sent on 1.15.20.
# This file will be provided by Brent each _____;
# *removed duplicate entries of LNU and MV5 and saved csv file with date of 1125;
# *There are now 1758 entries in the Sunjectline insert csv file received on 06.30.2020;
...
miss_inserts <- weighted_sample[is.na(weighted_sample$sv_subject_line_insert),]
...
```{r}
ia_subline_freq <- weighted_sample %>% 
...
# Merge data
weighted_sample <- weighted_sample %>% left_join(card_art_info)

sp_sv_card_freq <- weighted_sample %>% 
  freq_table(c("marketer_code","sv_card_art"), "SP Code vs SV Card Freq Table")
```

```{r}
sp135 <- weighted_sample %>% filter(marketer_code == 'SP135')
other <- weighted_sample %>% filter(marketer_code != 'SP135')
```

```{r message=F}
# Reading the file
centurion_tiers <- read_csv(CENTURION_TIER_PATH) %>% 
  set_names(c('gmpi_base_cust_id', 'CCSG_CENTURION_TIER')) %>%
  distinct(gmpi_base_cust_id, .keep_all = TRUE)

# Merging the dataframes
final <- sp135 %>% 
  left_join(centurion_tiers) %>% 
  full_join(other)
final <- final %>% set_names(str_to_upper(names(final))) %>% 
...
```{r}
final %>% write_csv("../Files_to_send/Amex_760_{MONTH}_{YEAR}_All_Sample_Frame_Information_Qualtrics_{Sys.Date()}.csv" %>% f_string(), na ="")

varhandle::rm.all.but(keep=c('final'))
```
